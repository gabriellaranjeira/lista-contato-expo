...
export const editaContato = (id, nomeContato, numeroContato, imageUri)=>{
    const promise = new Promise((resolve, reject)=>{
        db.transaction((tx)=>{
                tx.executeSql(
                'UPDATE tb_contato SET nome = ?, numero = ?, imageUri = ? WHERE id = ?;',
                [nomeContato, numeroContato, imageUri, id],
                (_,resultado)=>{resolve(resultado)},
                (_,err)=>{reject(err)}
            );
        });
